...
class DocumentTemplate:
    ...
    def get_partial(
        ...
        :param:
        context - словарь вида {field.tag: field.value}
        context_default - словарь значений по умолчанию {field.tag: field.name}
    ...
    def get_tags(self) -> List[str]:
        """Ищет и возвращает список тэгов подстановок переменных из шаблона"""
    ...
    def _print_document_runs(self, docx: Document):
        """
        Для исследования документа и печати всех его run.
        """
        invalid_runs = []
        for p in self._docx_paragraphs(docx):
            print(f"p=<{p.text}>")
...
if __name__ == "__main__":
    ...
    # path = "D:\\Dev\\document-template\\backend\\data\\"
    ...
    #     "ФИОРуководителя": "ФИО руководителя",
    #     "ФИО": "ФИО",
    #     "продолжительность": "продолжительность",
    #     "датаНачала": "дата начала",
    #     "ДолжностьЗаявителя": "должность заявителя",
    ...
    #     "ФИОРуководителя": "рогов федор федорович",
    #     "ФИО": "иванов иван иванович",
    #     "продолжительность": "14",
    #     "датаНачала": "23.10.2023",
    #     "ДолжностьЗаявителя": "старший рогополировальщик",
    #     "дата": "16.10.2023",
    #     "отпускные": 5000,
    # }
    ...
    path = "D:\\Dev\\document-template\\backend\\data\\"
    template_file_name = "претензия_tpl.docx"
    draft_file_name = "претензия_draft.docx"
    final_file_name = "претензия_final.docx"
    partial_file_name = "претензия_partial.docx"

    # Контекст для эскиза: словарь вида {field.tag: field.name}
    context_draft = {
        "Адресат": "Адресат (наименование организации или фамилия, имя, отчество исполнителя)",
        "Адрес": "Адрес (почтовый адрес исполнителя)",
        "Отправитель": "Фамилия, имя, отчество (укажите в родительном падеже)",
        "АдресОтправителя": "Почтовый адрес отправителя",
        "ДатаДоговора": "Дата заключения договора (день месяц год)",
        "ФИОЗаказчика": "Ф.И.О./наименование заказчика (укажите в творительном падеже)",
        "ФИОИсполнителя": "Ф.И.О./наименование исполнителя (укажите в творительном падеже)",
        "НомерДоговора": "Номер договора",
        "СуммаДоговора": "Сумма в рублях",
        "КвитанцияЧек": "Квитанция, чек об оплате (в творительном падеже) с датой",
        "СутьПретензии": "Суть претензии",
        "СтатусИсполнения": "Статус исполнения (не исполнены, исполнены не в полном объеме)",
        "ДоказательстваНеисполнения": "Доказательства неисполнения обязательств",
        "СуммаУбытков": "Сумма убытков в рублях",
        "СрокИсполнения": "Ожидаемый срок исполнения требований",
        "СодержаниеТребований": "Содержание предъявляемых требований",
        "ДокументыПриложения": "Документы, прикладываемые к претензии",
    }

    # Контекст для документа: словарь вида {field.tag: field.value}
    context_doc = {
        "Адресат": "Стогниенко Петр Андреевич",
        "Адрес": "г. Москва, ул.Радужная, д.150, офис.14",
        "Отправитель": "Иванов Иван Иванович",
        "АдресОтправителя": "г. Москва, пр-т.Центральный, д.5, кв.4",
        "ДатаДоговора": "01.10.2023",
        "ФИОЗаказчика": "Иванов Иван Иванович",
        "ФИОИсполнителя": "Стогниенко Петр Андреевич",
        "НомерДоговора": "22",
        "СуммаДоговора": "2500",
        "КвитанцияЧек": "чеком об оплате от 01.10.2023",
        "СутьПретензии": "выступить в качестве аниматора на дне рождения ребенка в костюме Человек-Паук, но аниматор выступил в костюме Бэтмен. Это даже не герой Marvel, а герой DC. Ребенок был сильно расстроен",
        "СтатусИсполнения": "не исполнены",
        "ДоказательстваНеисполнения": "фотографией выступления в костюме Бэтмен",
        "СуммаУбытков": "2500",
        "СрокИсполнения": "20.10.2023",
        "СодержаниеТребований": "Провести развлекательную программу с детьми в костюме Человек-Паук",
        "ДокументыПриложения": "чек об оплате заключенного договора, фотография с выступления на дне рождения",
    }
